...
def fetch_vn30_price_df() -> DataFrame:
    input_file = "data/raw/vn30_symbol_df.csv"
    output_file = "data/raw/vn30_price_df.csv"
    vn30_symbol_list = []

    if os.path.exists(input_file) == False:
        vn30_symbol_df = fetch_vn30_symbol_df()
        vn30_symbol_df.to_csv(input_file, index=False, encoding="utf-8-sig")
        vn30_symbol_list = vn30_symbol_df['symbol'].to_list()
    else:
        vn30_symbol_df = pd.read_csv(input_file)
        vn30_symbol_list = vn30_symbol_df['symbol'].to_list()
    ...
    vn30_price_df.to_csv(output_file, index=False)
...
vn30_index = fetch_vn30_index()
vn30_index.to_csv("data/raw/vn30_index_df.csv")
